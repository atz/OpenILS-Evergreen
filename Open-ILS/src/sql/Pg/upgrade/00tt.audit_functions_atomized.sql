...
DROP FUNCTION IF EXISTS auditor.create_auditor; -- Besides this line, the rest of this file is 900.audit-functions.sql
...
BEGIN
	EXECUTE $$
        SELECT auditor.create_auditor_seq            (sch, tbl);
        SELECT auditor.create_auditor_history        (sch, tbl);
        SELECT auditor.create_auditor_func           (sch, tbl);
        SELECT auditor.create_auditor_update_trigger (sch, tbl);
        SELECT auditor.create_auditor_lifecycle      (sch, tbl);
	$$;
	RETURN TRUE;
END;
$creator$ LANGUAGE 'plpgsql';
